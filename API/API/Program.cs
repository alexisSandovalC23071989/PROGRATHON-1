...
using (var scope = app.Services.CreateScope())
{
    ...
    // Agregar laboratorios si la lista est� vac�a
    if (!labsRepo.ObtenerTodos().Any())
    {
        ...
        labsRepo.Agregar(new LaboratorioModel { Nombre = "Laboratorio B", Capacidad = 20, Responsable = "Dra. G�mez" });
    }

    // Agregar usuarios si la lista est� vac�a
    if (!usuariosRepo.ObtenerTodos().Any())
    {
        usuariosRepo.Agregar(new UsuarioModel { Nombre = "Carlos P�rez", Tipo = "Estudiante", Correo = "carlos@universidad.com" });
        usuariosRepo.Agregar(new UsuarioModel { Nombre = "Ana L�pez", Tipo = "Profesor", Correo = "ana.lopez@universidad.com" });
    }

    // Agregar una reserva de ejemplo si la lista est� vac�a
